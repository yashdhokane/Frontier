        ...
        var appendedUsersParticipant = [];
            ...
            function initializeAutocompleteParticipant() {
                $("#participants").autocomplete({
                    source: function(request, response) {
                        $.ajax({
                            ...
                            success: function(data) {
                                var uniqueUsers = [];
                                data.forEach(function(item) {
                                    if (!appendedUsersParticipant.includes(item
                                            .value)) {
                                        uniqueUsers.push(item);
                                        appendedUsersParticipant.push(item
                                            .value);
                                    }
                                });
                                response(uniqueUsers);
                    ...
                    return $("<li>").addClass("col-md-8").text(item.label).attr("data-user-id",item.value).appendTo(
                ...
                $("#participants").on("input", function() {
                    ...
                    if (inputText === "") {
                        $("#autocomplete-results-users-part").empty();
                        appendedUsersParticipant = []; // Clear appended users when input is cleared
                    }
                });
            }
