                        ...
                        <a href="javascript:void(0)" class="chat-user message-item px-2">
                            {{-- <div class="d-flex align-items-center user-img1">
                                ...
                                <img src="{{ asset('public/images/technician/' . $value->user->user_image) }}"
                                    class="rounded-circle me-n2 card-hover border border-2 border-white user-img"
                                    width="35">
                                {{-- {{ asset('public/images/uploads/users/' . $user->id . '/' . $user->user_image) }}
                                --}}
                                {{-- @endif
                                @endforeach
                            </div> --}}
                            <div class="d-flex align-items-center user-img1">
                                @foreach ($item->Participant as $value)
                                @if ($value->user && $value->user->user_image)
                                <img src="{{ asset('public/images/Uploads/users/' . $value->user->id . '/' . $value->user->user_image) }}"
                                    class="rounded-circle me-n2 card-hover border border-2 border-white user-img"
                                    width="35">
                                @else
                        ...
                        <div class="card-body">
                            <div class="chat-meta-user-top d-flex pb-3 border-bottom">
                                <div class="current-chat-user-name">
                                    <span class="user-container1">

                                        <span class="name fw-bold ms-2"></span>
                                    </span>
                                </div>
                            </div>
                            <!-- <h4 class="card-title">Chat Messages</h4> this for chat message
                         -->
                                    ...
                                    <form action="{{ route('addUserToConversation') }}" method="post">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title">Add Users to Conversation</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <!-- Modal Body -->
                                            <div class="modal-body">
                                                <!-- Multi-select options go here -->
                                                <div class="col-sm-12 col-md-12">
                                                    <div class="mb-3">

                                                        <label for="User"
                                                            class="control-label bold mb5 col-form-label required-field">Search
                                                            User</label>
                                                        <input type="text" class="form-control" id="user" name="users"
                                                            required />
...
console.log(attach);
$('.chat-list').empty();
$('.chat-meta-user').empty(); // Clear previous user data
$('.chat-meta-user-top').empty(); // Clear previous user data

// Append user data to a container
$.each(partician, function(index, participant) {
var userImageSrc = participant.user.user_image ? 'public/images/Uploads/users/' + participant.user.id + '/' +
participant.user.user_image : '{{ asset('public/images/login_img_bydefault.png') }}';
...
'<div class=" align-items-center mb-3">' +
    '<img src="' + imageSrc + '" alt="avatar" class="rounded-circle me-2" width="40">' +
    '<div class="user-meta-info">' +
        '</div>' +
    '</div>'
);
...
var fileExtensions = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt','jfif', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'mp3', 'mp4',
...
// Function to check if a string is a file path (ends with a file extension)
function isFilePath(message) {
var fileExtensions = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'mp3', 'mp4',
'avi', 'mov']; // Add more file extensions if needed
var extension = message.split('.').pop().toLowerCase(); // Get the file extension from the message
return fileExtensions.includes(extension); // Check if the extension is in the list of file extensions
}
