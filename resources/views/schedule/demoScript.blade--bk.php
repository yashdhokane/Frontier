@section('script')
  <!--  <script
    ...
    if (job.job_model && Array.isArray(job.job_model.fieldids) && job.job_model.fieldids.length > 0) {
        // Join the field names into a single string
        fieldNames = job.job_model.fieldids.map(function(f) {
            return f.field_name;
        }).join(', ');
    }

    // Conditionally add the badge if fieldNames is not empty
    var fieldNamesBadge = fieldNames ? `<span class="badge bg-primary">${fieldNames}</span>` : '';
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                Swal.fire('Success', 'The duration has been updated successfully', 'success')
                                    .then(() => {});
                            },
                            error: function(xhr, status, error) {
