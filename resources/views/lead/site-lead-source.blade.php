        ...
        <div class="page-breadcrumb">
          <div class="row">
            ...
            <div class="col-3 align-self-center">
			     ...
			     <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#samedata-modal" data-bs-whatever="@mdo"><i class="ri-folder-add-line"></i> Add New</button>
							...
							<div class="table-responsive">
								...
								@foreach($leadSources as $leadSource)
									<tr>
                                        ...
                                        <!-- <td>{{ $leadSource->updated_by }}</td> -->
										<td>{{ $leadSource->created_at->format('m-d-Y') }}</td>
										<td>{{ $leadSource->count }}</td>
										<td>
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								<form id="updateLeadSourceForm"> 
									...
									<input type="hidden" id="lead-source-id" name="lead-source-id" value="{{ $leadSource->source_id }}"> 
								</form>
							</div>
							<div class="modal-footer">
								<button type="button" class="btn btn-light-danger text-danger font-medium" data-bs-dismiss="modal">Close</button>
...
<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			$('#updateLeadSourceBtn').on('click', function () {
				...
				$.ajax({
					url: '{{ url("/update-site-lead-source/") }}' + '/' + leadSourceId,
					type: 'POST',
					headers: {
						'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
					},
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).on('click', '.delete-btn', function () {
            ...
            if (confirm('Are you sure you want to delete ' + leadSourceName + '?')) {
                $.ajax({
                    url: '{{ url("/delete-site-lead-source/") }}' + '/' + leadSourceId,
                    type: 'DELETE', 
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
