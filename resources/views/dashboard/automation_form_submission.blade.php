...
<script>
 ...
 <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const formFields = [
            'first_name', 'last_name', 'mobile_phone', 'address1',
            'city', 'state_id', 'zip_code', 'home_phone', 
            'work_phone', 'company', 'user_type', 'role', 
            'additional_email', 'customer_notes'
        ];

        // Include URL generation for the route
        const fetchDataUrl = @json(route('autom.fetch.user'));

        emailField.addEventListener('blur', function() {
            const email = emailField.value;
            if (email) {
                fetch(`${fetchDataUrl}?email=${email}`)
                            ...
                            formFields.forEach(field => {
                                ...
                                if (fieldElement) {
                                    if (field === 'tag_id') {
                                        ...
                                        // Example: If you have a multi-select or checkboxes for tags
                                        const tagElements = document.querySelectorAll('.tag-checkbox');
                                        tagElements.forEach(tagElement => {
                                            tagElement.checked = data.tag_id.includes(parseInt(tagElement.value));
                                        });
                                    } else {
