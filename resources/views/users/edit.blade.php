      ...
      <div class="modal-body">
        ...
        @csrf
         <input type="hidden" class="form-control" name="id"
                                        value="{{$user->id}}" placeholder="" required />
...
<form method="POST" action="{{route('users.update', $user->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                         ...
                         <div class="row">
                            <div class="col-sm-12 col-md-8">
                                <div class="mb-3">
                                    <label for="address_unit" class="control-label col-form-label required-field">Address Line2</label>
                            ...
                            <div class="col-sm-12 col-md-4">
							<div class="mb-3">
    <label for="anotheraddress_type" class="control-label col-form-label required-field">Type</label>
            ...
            {{-- edit work address--}}
                <div id="adresscardtwo" class="card-body border-top" style="display:none;">
                    <br>
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-12">
                                <div class="mb-3">                                  <label for="address1" class="control-label col-form-label">Address Line 1
                                        (Street)</label>
                                    <input type="text" class="form-control" name="anotheraddress1"
                                        value="{{ old('address1', $location1->address_line1 ?? null) }}" placeholder=""
                                         />
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-8">
                                <div class="mb-3">
                                    <label for="address_unit" class="control-label col-form-label">Address Line2</label>
                                    <input type="text" class="form-control" name="anotheraddress_unit"
                                        value="{{ old('address_unit', $location1->address_line2 ?? null) }}"
                                        placeholder="" />
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-4">
							<div class="mb-3">
    <label for="anotheraddress_type" class="control-label col-form-label">Type</label>
    <select class="form-select me-sm-2" name="anotheraddress_type">
        <option value="">Select Address Type...</option>
        <option value="home" {{ ($location1->address_type ?? null) == 'home' ? 'selected' : '' }}>Home Address</option>
        <option value="work" {{ ($location1->address_type ?? null) == 'work' ? 'selected' : '' }}>Work Address</option>
        <option value="other" {{ ($location1->address_type ?? null) == 'other' ? 'selected' : '' }}>Other Address</option>
    </select>
</div>
							</div>
                        </div>
    ...
    <div class="mb-3">
        ...
        <select class="form-select me-sm-2" id="anotherstate_id" name="anotherstate_id" >
            <option selected disabled value="">Select State...</option>
            @foreach($locationStates as $locationState)
                <option value="{{ $locationState->state_id }}" {{ ($location1->state_id ?? null) == $locationState->state_id ? 'selected' : '' }}>
                    {{ $locationState->state_name }}
                </option>
            @endforeach
        </select>
   ...
   <div class="mb-3">
        ...
        <select class="form-select" id="anothercity" name="anothercity" >
            <option selected disabled value="">Select City...</option>
              @foreach($cities as $locationState)
                <option value="{{ $locationState->city_id }}" {{ ($location1->city ?? null) == $locationState->city_id ? 'selected' : '' }}>
                    {{ $locationState->city }}
                </option>
            @endforeach
        </select>
                            ...
                            <div class="col-sm-12 col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="anotherzip_code"  name="anotherzip_code"
                                        value="{{ old('zip_code', $location1->zipcode ?? null) }}" placeholder=""
                                         />
                                </div>
                            </div>
                        </div>
...
<div class="row">
    <div class="col-sm-12 col-md-12">
        <div class="mb-3">
            ...
            <select class="form-control js-example-basic-multiple" id="tag_id" name="tag_id[]" multiple>
                @foreach($tags as $tag)
                    <option value="{{ $tag->tag_id }}" 
                        {{ (in_array($tag->tag_id, old('tag_id', $userTags->pluck('tag_id')->toArray()))) ? 'selected' : '' }}>
                        {{ $tag->tag_name }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="p-3 border-top">
                <div class="action-form">
                    <div class="mb-3 mb-0 text-center">
                       ...
                       <a href="https://gaffis.in/frontier/website/users">  <button type="button" class="btn btn-dark rounded-pill px-4 waves-effect waves-light">Cancel</button> </a>
                    </div>
                </div>
            </div>
        </div>
